...
class PageRange:
    ...
    def acquire_lock(self, rid, query_opp):
        ...
        #return lstore.globals.fakeLockManager.getLock(self.table_name, self.index.read(rid), query_opp) # FIXME

    # def insert_acquire_lock(self, rid):
    #     address = Address(self.prid, 0, self.bOffSet)
    #     address.row = (rid - 1) % 511 + 1
    #     #return lstore.globals.lockManager.add_lock(INSERT, self.table_name, address) #FIXME
    #     print("Insert for: " + str(rid)) #FIXME print debug
    #     return lstore.globals.fakeLockManager.getLock(self.table_name, address, INSERT)
